...
/**
 * 非核心埋点逐级补充算法 V6 —— nUV³/PV² 排序公式。
 *
 * 相比 V5 改动：
 *   1. 排序公式由 nUV²/PV 改为 nUV³/PV²，更激进地偏向高覆盖、低PV事件
 *   2. 其余逻辑与 V5 保持一致
 */
public class EventSupplementByMinPvEntry_V6 {
    ...
    public static void main(String[] args) {
        ...
        SparkSession spark = SparkSession.builder()
                .appName("EventSupplementByMinPvEntry_V6_" + dt + "_" + bg)
                .enableHiveSupport()
        ...
        while (activeCount > 0 && step < topN) {
            ...
            for (int i = 0; i < activeCount; ) {
                ...
                double score = (double) nUV * nUV * nUV / (eventPVs[eid] * eventPVs[eid]);
                if (score > bestScore) {
                    bestScore = score;
                    bestId = eid;
                    bestIdx = i;
