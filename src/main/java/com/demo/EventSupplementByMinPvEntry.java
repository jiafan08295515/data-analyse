...
/**
 * 非核心埋点逐级补充算法 —— 从仅核心事件出发，每次贪心选择「单位PV换来最多新UV」的事件加入。
 *
 * 与裁剪 (EventPruneByMinPvAnchor) 对称：
 *   裁剪做减法：删除事件 → 衡量 UV 损失率
 *   补充分加法：添加事件 → 衡量 UV 覆盖率
 */
public class EventSupplementByMinPvEntry {
    ...
    public static void main(String[] args) {
        ...
        SparkSession spark = SparkSession.builder()
                .appName("EventSupplementByMinPvEntry_" + dt + "_" + bg)
                .enableHiveSupport()
        ...
        // ========== 3. 展开非核心埋点用户映射 ==========
        // 取出 rank>0 的埋点，展开 imei_info_list → (event_key, imei, event_pv)
        ...
        // 埋点元数据 (event_key, pagetype, actiontype, event_pv)
        ...
        // 构建 Map<event_key, Set<imei>> 和 PV 查找表
        Map<String, Set<String>> eventUsersMap = new HashMap<>();
        Map<String, Long> eventPVMap = new HashMap<>();
        Map<String, String> eventPagetypeMap = new HashMap<>();
        Map<String, String> eventActiontypeMap = new HashMap<>();
        ...
        for (Row r : eventMeta) {
            ...
            eventPVMap.put(key, pv);
            eventUsersMap.put(key, new HashSet<String>());
            eventPagetypeMap.put(key, pagetype);
            eventActiontypeMap.put(key, actiontype);
        }

        for (Row r : userPairs) {
            String key = r.getString(0);
            String imei = r.getString(1);
            Set<String> users = eventUsersMap.get(key);
            if (users != null) {
                users.add(imei);
            }
        }
        ...
        // ========== 6. Driver 端贪心选择 ==========
        Set<String> covered = new HashSet<>();           // 已被覆盖的非核心用户
        Set<String> remaining = new HashSet<>(eventUsersMap.keySet());
        ...
        while (!remaining.isEmpty() && step < topN) {
            // 对每个剩余事件计算「新增 UV / PV」性价比
            String bestKey = null;
            double bestScore = -1;
            long bestNewUV = 0;
            ...
            for (String key : remaining) {
                ...
                long newUV = 0;
                for (String u : users) {
                    if (!covered.contains(u)) newUV++;
                }
                if (newUV == 0) continue;
                long pv = eventPVMap.get(key);
                double score = (double) newUV / pv;
                if (score > bestScore) {
                    bestScore = score;
                    bestKey = key;
                    bestNewUV = newUV;
            ...
            if (bestKey == null) break; // 剩余事件都无新用户

            // 选中该事件
            step++;
            covered.addAll(eventUsersMap.get(bestKey));
            cumUV += bestNewUV;
            cumPV += eventPVMap.get(bestKey);
            remaining.remove(bestKey);

            Map<String, Object> row = new LinkedHashMap<>();
            row.put("supplement_step", step);
            row.put("supplemented_pagetype", eventPagetypeMap.get(bestKey));
            row.put("supplemented_actiontype", eventActiontypeMap.get(bestKey));
            row.put("supplemented_event_pv", eventPVMap.get(bestKey));
            ...
            System.out.printf("Step %3d | %-8s %-20s %-25s | PV=+%8d | 新UV=+%8d | 累积UV=%10d (%.2f%%) | 累积PV=%15d (%6.2f%%)%n",
                    step, "[补充]", eventPagetypeMap.get(bestKey), eventActiontypeMap.get(bestKey),
                    eventPVMap.get(bestKey), bestNewUV,
                    cumUV, (double) cumUV / totalUV * 100,
                    cumPV, (double) (cumPV - corePV) / totalPV * 100);
            ...
            while (it.hasNext()) {
                String k = it.next();
                long remainingNew = 0;
                for (String u : eventUsersMap.get(k)) {
                    if (!covered.contains(u)) remainingNew++;
                }
                if (remainingNew == 0) it.remove();
            }
        }
